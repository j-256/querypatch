...
export const RULE_MATCH_STATUSES = Object.freeze({
  EMPTY: "empty",
  EXCLUDED: "excluded",
...
export function evaluateRuleMatch(value, urlValue) {
  ...
  let url;
  try {
    url = parseTestUrl(rawUrl);
  } catch (error) {
    const detail = error instanceof RuleValidationError
      ? error.issues[0]?.message || error.message
      : error.message;
    return ruleMatchResult(RULE_MATCH_STATUSES.INVALID_URL, detail);
  }

  if (rule.includes.length === 0) {
    return ruleMatchResult(
      RULE_MATCH_STATUSES.INVALID_PATTERN,
      "Add at least one match pattern",
    );
  }

  const patternGroups = [
    ["Match pattern", rule.includes],
    ["Exclusion", rule.excludes],
  ];

  for (const [label, patterns] of patternGroups) {
  ...
  if (!includePattern) {
    return ruleMatchResult(
      RULE_MATCH_STATUSES.MISS,
      "No match pattern accepted this URL",
    );
  }
  ...
  return ruleMatchResult(
    RULE_MATCH_STATUSES.MATCH,
    `Matched ${includePattern}`,
    { includePattern },
  );
}

function ruleMatchResult(status, detail, patterns = {}) {
  return {
    status,
    matches: status === RULE_MATCH_STATUSES.MATCH,
    detail,
    ...patterns,
  };
}
